...
class FootwearModel(Model):
    def __init__(self):
        self.model = self.load_or_train_model(MODELS_FILENAME[FOOTWEAR])
    
    def train_model(self, model, train, val, epochs=EPOCHS_FOOTWEAR, steps_per_epoch=STEPS_PER_EPOCH_FOOTWEAR, model_name=FOOTWEAR):
        return super().train_model(model, train, val, epochs, steps_per_epoch, model_name)
    
    def get_model_prediction(self, data) -> str:
        predictions = self.model.predict(data)
        print("get model prediction - footwear")
    ...
    def get_dataframe(
        ...
        print("-"*40)
        print("Label encoders - footwear model")
