...
class BottomwearModel(Model):
    def __init__(self):
        self.model = self.load_or_train_model(MODELS_FILENAME[BOTTOMWEAR])
    
    def train_model(self, model, train, val, epochs=EPOCHS_BOTTOMWEAR, steps_per_epoch=STEPS_PER_EPOCH_BOTTOMWEAR, model_name=BOTTOMWEAR):
        return super().train_model(model, train, val, epochs, steps_per_epoch, model_name)

    def get_model_prediction(self, data) -> str:
        predictions = self.model.predict(data)
        print("get model prediction - bottomwear")
    ...
    def get_dataframe(
        ...
        print("-"*40)
        print("Label encoders - bottomwear model")
